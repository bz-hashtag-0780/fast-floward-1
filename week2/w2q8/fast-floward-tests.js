	...
	describe('Fast Floward Tests', async () => {
		...
		it(`safely fails when withdrawing kittyitem with wrong id`, async () => {
			...
			try {
				...
				let res1 = await DappLib.kittyItemsReadCollectionLength(
					testData2
				);
				let res2 = await DappLib.kittyItemsReadCollectionLength(
					testData3
				);
		...
		it(`lists 2 kittyitems for sale`, async () => {
			...
			let res1 = await DappLib.kittyItemsMarketReadSaleCollectionIDs(
				testData3
			);
		...
		it(`removes a kittyitem from sale`, async () => {
			...
			let res1 = await DappLib.kittyItemsMarketReadSaleCollectionLength(
				testData2
			);
			// 6) call kittyItemsMarketReadSaleCollectionIDs with testData3 and store the result
			let res2 = await DappLib.kittyItemsMarketReadSaleCollectionIDs(
				testData3
			);
			...
			assert.equal(
				res1.result,
				1,
				'User should have 1 KittyItems up for sale'
			);
